...
// PMSettings.jsx — mobile-first tidy layout
...
function Avatar({ user }) {
  const initials = (() => {
    const name = user?.name || user?.full_name || user?.username || user?.email || "NS";
      ...
      <div className="pmo-page-header flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="pmo-page-subtitle">{user?.name || "Account"}</p>
          </div>
          {user?.company_name && (
              ...
              <div className="space-y-0 divide-y divide-slate-50">
                {[
                  ...
                  <div key={label} className="flex items-baseline gap-3 py-3">
                    <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide" style={{ minWidth: "5.5rem", flexShrink: 0 }}>{label}</span>
